export function validateBooking(data) {
  ...
  if (!data.email.trim() || !emailOk)
    errs.email = "Kindly provide a valid email address.";
  ...
  if (!data.pickupTime?.trim())
    errs.pickupTime = "Please choose a pickup time.";
  ...
  // Vehicle
  if (!data.vehicle)
    errs.vehicle = "Please choose your preferred vehicle class.";
